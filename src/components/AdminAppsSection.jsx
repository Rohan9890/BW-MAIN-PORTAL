...
function mergeAssetResponseIntoRow(row, res, { hadLogo, hadBanner }) {
  ...
  if (hadLogo) {
    const u =
      (data && String(data.logoUrl || "").trim()) ||
      (hadBanner ? "" : unwrapAssetUploadUrl(res));
    if (u) next.logoUrl = u;
  }
  if (hadBanner) {
    const u = data && String(data.bannerUrl || "").trim();
  ...
  const uploadAssets = async (appId, logoFile, bannerFile) => {
    ...
    try {
      ...
      setApps((prev) =>
        prev.map((r) =>
          r.id === String(appId)
            ? mergeAssetResponseIntoRow(r, res, { hadLogo: !!logoFile, hadBanner: !!bannerFile })
            : r,
        ),
      );
    ...
    try {
      ...
      if (isNew) {
        ...
        if (id && (pendingLogoFile || pendingBannerFile)) {
          await uploadAssets(id, pendingLogoFile, pendingBannerFile);
        }
        showSuccess("App created");
      } else {
        await adminDashboardApi.updateAdminApp(editorDraft.id, body);
        if (pendingLogoFile || pendingBannerFile) {
          await uploadAssets(editorDraft.id, pendingLogoFile, pendingBannerFile);
        }
                  ...
                  <tr key={app.id}>
                    <td>
                      <div className="apps-logo-cell">
                        {app.logoUrl ? (
                          <img src={resolveAppMediaUrl(app.logoUrl)} alt="" loading="lazy" />
                        ) : (
              ...
              <div className="admin-apps-two">
                <div>
                  ...
                  <div className="admin-apps-thumb">
                    {logoDisplay ? <img src={logoDisplay} alt="" loading="lazy" /> : <span>No logo</span>}
                  </div>
                </div>
                <div>
                  ...
                  <div className="admin-apps-thumb admin-apps-thumb-wide">
                    {bannerDisplay ? <img src={bannerDisplay} alt="" loading="lazy" /> : <span>No banner</span>}
                  </div>
                </div>
              </div>
            </div>
